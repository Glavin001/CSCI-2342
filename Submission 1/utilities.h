...
The source code that makes use of this utilities package must include
this header file and link to the corresponding object file (which
is called utilities.obj under Windows XP, for example).

The package contains features on four categories, as listed below.
...
Fifteen free functions:
- ClearScreen
- DisplayOpeningScreen (inline)
- DisplayTextfile
- gcd
- isEven
- isOdd
...
the user for a value of the correct type if one is not entered
...
/**
@mainpage
This is the Fall, 2012 version of the documentation for the
...
is available, but you need to start using the package immediately
and learn some new facilities each week as the term progresses.
@section whatsnew_sec What's New
Those who are familiar with this utilities package from last year
should note the changes and additions listed below. Those who may
have used it previous to that will find more significant changes and
they should review the documentation in its entirety, as should those
who are encountering the package for the first time.
-# The DisplayIDInfo() free function has been renamed DisplayOpeningScreen(),
but retains essentially the same functionality. This function is also now
an inline function, which helps to avoid multiply-defined function name
compile-time errors when utilities.h is included in several different source
files of the same program. The two named constants which provide values for
the default parameters of the DisplayOpeningScreen() function have also been
renamed: DEFAULT_DESCRIPTION is now called DEFAULT_PROGRAM_INFO, and
DEFAULT_ID_INFO is now called DEFAULT_PROGRAMMER_INFO.
-# There is a new ClearScreen() function that clears the screen and positions
the cursor to write at the bottom left corner of the screen. This a portable
function that works simply by writing blank lines to the screen.
-# The ReadLine() free function has been replaced by two very similar
free functions, the ReadNextLine() function and the ReadThisLine() function.
The ReadNextLine() function displays a prompt and moves the cursor to the
next line for the user to enter as much as a full line of text, while the
ReadThisLine() function displays a prompt and keeps the cursor on the same
line as that prompt so that the user can enter (a presumably shorter piece
of) text on the same line as the prompt. Note that the ReadNextLine()
function has exactly the same functionality as the previous ReadLine()
function. In both cases all text entered, except the terminating newline
character, is read into the string variable receiving the input.
-# There are three new constant arrays containing names: one containing
...
at these sites:
<br>www.census.gov/genealogy/names/dist.female.first
<br>names.mongabay.com/male_names_alpha.htm
<br>en.wikipedia.org/wiki/List_of_most_common_surnames#United_States
*/
...
question mark.
@pre <em>question</em> has been initialized.
